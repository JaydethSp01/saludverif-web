        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {(products ?? []).filter((p) => p.category === cat).map((p) => (
            <button key={p.id} onClick={() => add(p.id)}
              className="flex flex-col items-start rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/40">
